'use strict';
/* PONT AULATECH · contracte v1. El joc envia FETS, mai XP: l'autoritat
   econòmica és submit_game_result() al backend. La dificultat viu al catàleg. */
const Bridge = (function () {
  ...
  let cb = null, t0 = Date.now(); const sent = new Set();
  window.addEventListener('message', (e) => {
    const d = e.data || {};
    if (d.source === 'aulatech-host' && d.action === 'GAME_RESULT' && cb) cb(d.data || {});
  });
  return {
    startClock() { t0 = Date.now(); sent.clear(); },
    send(f) {
      f = f || {};
      const data = { p_juego_id: f.id || 'note-snake', p_bloque: 'notes', p_tema: null,
        p_completado: !!f.completat, p_precision: clamp01(f.precisio),
        p_errores: int(f.errors), p_racha_max: int(f.rachaMax),
        p_tiempo_ms: int(f.tempsMs === undefined ? Date.now() - t0 : f.tempsMs),
        p_perfecto: !!f.perfecte };
      try { window.parent.postMessage({ source: 'aulatech', action: 'GAME_END', v: 1, data }, '*'); }
      catch (e) {}
      return data;
    },
    sendOnce(f) { const k = (f && f.id) || 'x'; if (sent.has(k)) return null; sent.add(k); return this.send(f); },
    onResult(f) { cb = f; },
  };
})();

// `const` en un script clàssic viu a l'àmbit lèxic global, NO a window:
// sense aquesta línia, els jocs que criden `window.Bridge?.send(...)` no
// fallen —simplement no fan res— i la partida no arriba mai al servidor.
